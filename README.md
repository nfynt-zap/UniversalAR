...
Both ZapWorks dashboard and CLI now support the `Gzip` and `Brotli` compression format of Unity WebGL builds, so it's no longer necessary to enable `Publishing Settings > Decompression Fallback` in Unity2020 or above, or disabling `WebAssembly Streaming` in Unity 2019.
